defmodule UnirisCore.P2P.TransactionLoaderTest do
  ...
  test "when get {:new_transaction, %Transaction{type: :beacon} with node readyness information, the given node become ready",
       %{pid: pid} do
    P2P.add_node(%Node{
      ip: {127, 0, 0, 1},
      port: 3000,
    ...
    tx =
      Transaction.new(:beacon, %TransactionData{
        content: """
        N - #{Base.encode16(Crypto.node_public_key())} - R
        """
      })
