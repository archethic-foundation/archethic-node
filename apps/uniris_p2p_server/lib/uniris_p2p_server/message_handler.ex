defmodule UnirisP2PServer.MessageHandler do
  ...
  def process({:closest_nodes, network_patch}) do
    P2P.nearest_nodes(network_patch, P2P.list_nodes())
    |> Enum.take(5)
  end
  ...
  def process({:bootstrap_crypto_seeds, pub}) do
    ...
    %{
      ...
      authorized_nodes: Map.keys(authorized_keys)
    }
  end
  ...
  def process(
        {:start_mining, tx = %Transaction{}, welcome_node_public_key, validation_nodes}) do
  ...
  def process({:set_replication_tree, tx_address, tree}) do
    Validation.set_replication_tree(tx_address, tree)
  end
  ...
  def process({:acknowledge_storage, tx_address}) do
    UnirisSync.publish_storage(tx_address)
  end
