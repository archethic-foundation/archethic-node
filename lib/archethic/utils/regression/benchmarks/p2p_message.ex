defmodule ArchEthic.Utils.Regression.Benchmark.P2PMessage do
  ...
  @genesis_seed Application.compile_env!(:archethic, ArchEthic.Bootstrap.NetworkInit)[
                  :genesis_seed
                ]
  @address Crypto.derive_keypair(@genesis_seed, 1) |> elem(0) |> Crypto.hash()

  defp get_transaction(addr, port) do
    {:ok, socket} = connect(addr, port)
    :ok = send_msg(socket, %GetTransaction{address: @address})
    {:ok, %Transaction{}} = recv(socket)
  end

  defp get_transaction_chain(addr, port) do
    {:ok, socket} = connect(addr, port)
    :ok = send_msg(socket, %GetTransactionChain{address: @address})
