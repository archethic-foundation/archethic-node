defmodule ArchEthic.Utils.Benchmark.Helpers.Helper do
  @moduledoc """
  ...
  def get_genesis_address({pbKey, _privKey}), do: Crypto.derive_address(pbKey)
  ...
  def allocate_funds(recipient_address, host, port) do
    with {:ok, true} <- faucet_enabled?(),
      ...
      deploy_txn(:transfer, @pool_seed, recipient_address, 10_000_000_000, host, port)
    else
      _ -> {:error, :raise}
    end
  end
  ...
  def get_chain_size(seed, host, port) do
    genesis_address = seed |> derive_keypair() |> get_genesis_address()
  ...
  def deploy_txn(txn_type, emitter_seed, recipient_address, amount \\ 1_000_000, host, port) do
    ...
    dispatch_txn_to_public_endpoint(txn, host, port)
  end
