defmodule ArchEthic.P2P.Message do
  Module.register_attribute(__MODULE__, :messages_id, accumulate: true, persist: true)

  @callback encode(any()) :: bitstring()
  @callback decode(bitstring()) :: {any(), bitstring()}
  ...
  @on_load :init
  def init do
    :persistent_term.put(:message_ids, [])
    :ok
  end

  defmacro __using__(message_id: message_id) do
    quote do
      @behaviour ArchEthic.P2P.Message

      @on_load :register_message
      def register_message do
        ArchEthic.P2P.Message.register_message(unquote(message_id), __MODULE__)
      end
    end
  end

  def register_message(message_id, module) do
    message_ids = :persistent_term.get(:message_ids)

    case Enum.find(message_ids, fn {msg_id, _} -> msg_id == message_id end) do
      nil ->
        :persistent_term.put(:message_ids, [{message_id, module} | message_ids])
     ...
     {_, ^module} ->
      ...
      {_, existing_module} ->
        Logger.warning(
          "#{module}: Message id #{message_id} already assigned in #{existing_module}"
        )
    end
  end

  def get_message_id(module) do
    message_ids = :persistent_term.get(:message_ids)
    {msg_id, _} = Enum.find(message_ids, fn {_, mod} -> mod == module end)
    msg_id
  end

  def get_message_module(msg_id) do
    message_ids = :persistent_term.get(:message_ids)
    {_, mod} = Enum.find(message_ids, fn {id, _} -> msg_id == id end)
    mod
  end
  ...
  def encode(message) when is_struct(message) do
    ...
    <<msg_id::8, message.encode(message)::bitstring>>
  end

  @spec decode(bitstring()) :: {any(), bitstring()}
  def decode(<<message_id::8, rest::bitstring>>) do
    module = get_message_module(message_id)
    module.decode(rest)
  end

  @spec process(any()) :: any()
  def process(message) when is_struct(message) do
...
# defmodule ArchEthic.P2P.Message
# @moduledoc """
