defmodule ArchEthic.SelfRepair.Sync.BeaconSummaryHandler.NetworkStatistics do
  @moduledoc false
  ...
  def init(_opts) do
    ...
    unless File.exists?(dump_filename(:archethic_tps)) do
      :ets.new(:archethic_tps, [:named_table, :ordered_set, :public, read_concurrency: true])
    end

    unless File.exists?(dump_filename(:archethic_stats)) do
      :ets.new(:archethic_stats, [:named_table, :set, :public, read_concurrency: true])
    end

    :ets.file2tab(dump_filename(:archethic_tps) |> String.to_charlist())
    :ets.file2tab(dump_filename(:archethic_stats) |> String.to_charlist())
  ...
  def get_latest_tps do
    case :ets.last(:archethic_tps) do
      :"$end_of_table" ->
        0.0

      key ->
        [{_, tps, _nb_transactions}] = :ets.lookup(:archethic_tps, key)
        tps
    end
  end
  ...
  def get_nb_transactions do
    case :ets.lookup(:archethic_stats, :nb_transactions) do
      [] ->
        0
  ...
  def increment_number_transactions(nb \\ 1) when is_integer(nb) and nb >= 0 do
    new_nb = :ets.update_counter(:archethic_stats, :nb_transactions, nb, {0, 0})
    dump_table(:archethic_stats)
  ...
  def register_tps(date = %DateTime{}, tps, nb_transactions)
    ...
    true = :ets.insert(:archethic_tps, {date, tps, nb_transactions})
    :ok = dump_table(:archethic_tps)
  ...
  defp dump_dirname do
    dump_dir = Application.get_env(:archethic, __MODULE__) |> Keyword.fetch!(:dump_dir)
