defmodule ArchEthic.Replication.TransactionContext do
  ...
  """
  def fetch_complete_transaction_chain(address, timestamp = %DateTime{}, force_remote_download? \\ false)
    when is_binary(address) and is_boolean(force_remote_download?) do
      case replication_nodes(address, timestamp, force_remote_download?) do
        [] ->
          []
  ...
  """
  defp replicate_transaction_chain(nodes , address , time_after = %DateTime{} , page_state // nil , prev_result // nil )do

  defp replicate_transaction_chain([node | rest] , address , time_after = %DateTime{} , page_state // nil , _prev_result )do

    case P2P.send_message( node , %GetTransactionChain{address: address, after: time_after, page: page_state }) do
      # ------------------------------------------------------------------
      ...
      {:ok, %TransactionList{transactions: transactions , more?: true, page: page}} when page > 0  ->
          case process_transactions(transactions, address, time_after , page) do
            {:ok, new_time_after} -> replicate_transaction_chain([node | rest] , address , time_after, page , [])
            {:error, _ } ->  replicate_transaction_chain(rest , address , time_after, nil, [])
          end
      ...
      {:ok, %TransactionList{transactions: transactions , more?: false, page: page}} when page >= 0  ->
          case process_transactions(transactions, address, time_after , page) do
            {:ok, new_time_after} -> replicate_transaction_chain(rest , address , new_time_after, nil, [])
            {:error, _ } ->  replicate_transaction_chain(rest , address , time_after, nil, [])
          end
  ...
  defp replicate_transaction_chain([], _address, _time_after, _page_state, prev_result) do: {:ok , prev_result}
