defmodule Archethic.Contracts.WorkerV2 do
  @moduledoc false
  ...
  def handle_event(
        ...
        :start_schedulers,
        _state,
        data = %{contract: %ContractV2{instance: instance}}
      ) do
    %WasmSpec{triggers: triggers} = WasmInstance.spec(instance)
    triggers_type = Enum.map(triggers, & &1.type)
  ...
  def handle_event(
        :cast,
        {:new_contract, contract, _},
        :idle,
        data = %{contract: %ContractV2{}}
      ) do
  ...
  def handle_event(
        ...
        :reparse_contract,
        _,
        data = %{contract: %ContractV2{transaction: contract_tx}}
      ) do
    {:keep_state, %{data | contract: ContractV2.from_transaction!(contract_tx)}}
  end
  ...
  def handle_event(
        ...
        _state,
        data = %{
          contract: %ContractV2{transaction: %Transaction{address: contract_address}},
  ...
  defp handle_trigger(
         ...
         data = %{
           genesis_address: genesis_address,
           contract: contract = %ContractV2{instance: instance}
         }
       ) do
    unspent_outputs = fetch_unspent_outputs(genesis_address)
    ...
    case execute_contract(contract, trigger_type, nil, nil, genesis_address, unspent_outputs) do
      ...
      _ ->
        %WasmSpec{triggers: triggers} = WasmInstance.spec(instance)
        triggers_type = Enum.map(triggers, & &1.type)
        interval_timer = schedule_trigger({:interval, interval}, triggers_type)
  ...
  defp handle_trigger(
         ...
         data = %{
           genesis_address: genesis_address,
           contract: contract = %ContractV2{transaction: %Transaction{address: contract_address}}
         }
       ) do
  ...
  defp execute_contract(
    ...
    with {:ok, %ActionWithTransaction{next_tx: next_tx}} <-
         ...
         {:ok, next_tx} <- ContractV2.sign_next_transaction(contract, next_tx, index),
         contract_context <-
