defmodule Uniris.DB.CassandraImpl do
  ...
  def get_transaction(address, fields \\ []) when is_binary(address) and is_list(fields) do
    result =
      Producer.add_query(
        "SELECT #{CQL.list_to_cql(fields)} FROM uniris.transactions WHERE chain_address=? PER PARTITION LIMIT 1",
        [address]
      )
  ...
  def get_transaction_chain(address, fields \\ []) when is_binary(address) and is_list(fields) do
    ...
    |> Task.async_stream(fn bucket ->
      "SELECT #{CQL.list_to_cql(fields)} FROM uniris.transactions WHERE chain_address=? and bucket=?"
  ...
  defp do_write_transaction(
    ...
    "INSERT INTO uniris.transactions (chain_address, bucket, timestamp, version, address, type, data, previous_public_key, previous_signature, origin_signature, validation_stamp, cross_validation_stamps) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    |> Producer.add_query([
      chain_address,
      bucket,
      timestamp,
  ...
  def list_transactions_by_type(type, fields \\ []) do
    ...
    |> Task.async_stream(fn %{"address" => address} ->
      "SELECT #{CQL.list_to_cql(fields)} FROM uniris.transactions WHERE chain_address=?"
      |> Producer.add_query([address])
      |> Enum.at(0)
  ...
  def register_tps(date = %DateTime{}, tps, nb_transactions)
    ...
    Producer.add_query(
      "INSERT INTO uniris.network_stats_by_date (date, tps, nb_transactions) VALUES (?, ?, ?)",
