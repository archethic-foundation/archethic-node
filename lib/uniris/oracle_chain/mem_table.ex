defmodule Uniris.OracleChain.MemTable do
  ...
  def init(_) do
    :ets.new(:uniris_oracle, [:ordered_set, :named_table, :public, read_concurrency: true])
    {:ok, []}
  end
  ...
  def add_oracle_data(type, data, date = %DateTime{}) when is_map(data) do
    ...
    true = :ets.insert(:uniris_oracle, {{timestamp, type}, data})
    :ok
  end
  ...
  def get_oracle_data(type, date = %DateTime{}) do
    ...
    case :ets.lookup(:uniris_oracle, {timestamp, type}) do
      [] ->
        case :ets.prev(:uniris_oracle, {timestamp, type}) do
          :"$end_of_table" ->
            {:error, :not_found}

          key ->
            [{_, data}] = :ets.lookup(:uniris_oracle, key)
            {:ok, data}
        end
