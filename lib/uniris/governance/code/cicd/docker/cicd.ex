defmodule Uniris.Governance.Code.CICD.Docker do
  ...
  The service relies on the `Dockerfile` with two targets: `uniris-ci` and
  `uniris-cd`.

  The `uniris-ci` target produces an image with build tools. Its goal is to
  compile the source code into `uniris_node` release. The CI part is powered by
  `scripts/proposal_ci_job.sh`. The script runs in a container named
  `uniris-prop-{address}`, it produces: release upgrade of `uniris_node`, new
  version of `uniris-proposal-validator`, and combined log of application of a
  ...
  The `uniris-cd` target produces an image capable of running `uniris_node`
  release.
  """
  use Supervisor
  ...
  defp do_run_docker_ci(%Proposal{address: address, changes: changes}) do
    ...
    args = ["run", "--entrypoint", @ci_script, "-i", "--name", name, "uniris-ci", name]
  ...
  defp build_cd_image do
    {_, 0} = docker(["build", "-t", "uniris-cd", "."])
    :ok
  end

  @validator "/opt/code/uniris-proposal-validator"
  @releases "/opt/code/_build/dev/rel/uniris_node/releases"
  @release "uniris_node.tar.gz"
      ...
      |> Task.async_stream(
        fn c ->
          with {_, 0} <- docker(["exec", c, "mkdir", "-p", "#{dst}"]),
               ...
               {_, 0} <- docker(["exec", c, "./bin/uniris_node", "upgrade", version]) do
            :ok
          else
            error ->
  ...
  defp testnet_start(dir, seeds) do
    ...
    options = [image: "uniris-cd", dir: dir, src: @src_dir, persist: false]
