defmodule Uniris.P2P.Multiplexer.Demuxer do
  ...
  def init(args) do
    ...
    Task.start(fn ->
      Transport.read_from_socket(transport, socket, fn data ->
        responses = decode_data(data)
        recv_handler.(responses, muxer_pid: muxer_pid, multiplexer_pid: multiplexer_pid)
      end)
    end)
