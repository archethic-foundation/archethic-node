defmodule Mix.Tasks.Uniris.Testnet do
  ...
  def run(args) do
    case OptionParser.parse!(args,
           ...
           aliases: [h: :help, o: :output, n: :network, s: :subnet, i: :image]
         ) do
      {_, []} ->
  ...
  defp from(seeds, opts) do
    ...
    services =
      ...
      |> Enum.with_index(1)
      |> Enum.map(&to_node(&1, image, network, ip))
      |> Enum.into(%{})
  ...
  defp to_node({[seed], 1}, image, network, ip) do
    {"node1",
     %{
  ...
  defp to_node({[seed | seeds], n}, image, network, ip) do
    seeders =
      seeds
      |> Enum.reverse()
      |> Enum.with_index(1)
    ...
    {"node#{n}",
     %{
       ...
       volumes: ["./data#{n}:/opt/data", "./scripts/wait-for-node.sh:/wait.sh:ro"],
