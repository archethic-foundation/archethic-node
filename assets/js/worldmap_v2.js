...
function createMarkerCluster(nodes) {
...
export function createWorldmap(worldmapDatas) {
  ...
  const nodes = formatData(worldmapDatas);
  markerClusterGroup = createMarkerCluster(nodes);
...
export function updateWorldmap(worldmapDatas) {
  ...
  markerClusterGroup.clearLayers(); // ⬅️ évite le removeLayer + re-add
