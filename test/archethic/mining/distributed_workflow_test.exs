defmodule Archethic.Mining.DistributedWorkflowTest do
  ...
  test "should never have more validation nodes than storage", %{
    tx: tx,
    sorting_seed: sorting_seed
  } do
    validation_nodes =
    ...
    # Fake (3*2=6) validation nodes
    validation_nodes = Enum.concat([validation_nodes, validation_nodes, validation_nodes])
    assert 6 == length(validation_nodes)
    ...
    # 1 is the coordinator node
    assert length(chain_storage_nodes) >= 1 + length(cross_validation_nodes)
  end
