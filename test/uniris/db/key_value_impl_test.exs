defmodule Uniris.DB.KeyValueImplTest do
  ...
  test "add_last_transaction_address/2 should reference a last address for a chain", %{root_dir: root_dir} do
    {:ok, _pid} = KV.start_link(root_dir: root_dir)
    assert :ok = KV.add_last_transaction_address("@Alice1", "@Alice2")
  end

  test "list_last_transaction_addresses/0 should retrieve the last transaction addresses" , %{ root_dir: root_dir} do
    {:ok, _pid} = KV.start_link(root_dir: root_dir)
