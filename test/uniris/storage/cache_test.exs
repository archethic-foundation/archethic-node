defmodule Uniris.Storage.CacheTest do
  use UnirisCase
  ...
  describe "store_transaction/1" do
    test "should index the transaction as node transaction" do
      ...
      assert [validated_tx.address] == Cache.list_node_transaction_addresses()
    end
  ...
  test "last_transaction_address/1 should retrieve the last transaction on a chain" do
    ...
    assert {:ok, tx3.address} == Cache.get_last_transaction_address(tx1.address)
    assert {:ok, tx3.address} == Cache.get_last_transaction_address(tx2.address)
    assert {:ok, tx3.address} == Cache.get_last_transaction_address(tx3.address)
  end
